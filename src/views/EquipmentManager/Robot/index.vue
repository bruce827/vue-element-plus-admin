...
import { ElButton, ElTag, TableColumn } from 'element-plus'
import { Table } from '@/components/Table'
import { getTableListApi, saveTableApi, delTableListApi } from '@/api/equipmentManager'
  ...
  },
  {
    ...
    formatter: (_: Recordable, __: TableColumn, cellValue: string) => {
      return h(
        ElTag,
        {
          type: cellValue === '启用' ? 'success' : 'danger'
        },
        () => cellValue
      )
    },
    detail: {
      slots: {
        default: (data: any) => {
          return <ElTag type={data.状态 == '启用' ? 'success' : 'warning'}>{data.状态}</ElTag>
        }
      }
    }
  },
